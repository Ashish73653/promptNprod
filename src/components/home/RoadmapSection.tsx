    ...
    <section className="py-12 sm:py-16 bg-slate-50/50 dark:bg-slate-950/40 border-y border-slate-200/60 dark:border-slate-800/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-8">
          {/* Quick Filter Tabs — scrollable on mobile, no wrap */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredRoadmaps.map((roadmap) => {
              ...
              return (
                ...
                >
                  <div>
                    {/* Icon + badges — single row, no wrap */}
                    <div className="flex items-center justify-between gap-2 mb-4 min-w-0">
                      ...
                      <div className="flex items-center gap-1.5 overflow-hidden min-w-0">
                        <span className="text-xs px-2 py-0.5 rounded-full font-bold bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 border border-cyan-500/20 truncate max-w-[90px] sm:max-w-none">
                          {roadmap.badge}
                        </span>
                        {projectCount > 0 && (
                          <span className="hidden sm:inline-flex text-xs px-2 py-0.5 rounded-full font-semibold bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 items-center gap-1 shrink-0">
